  ...
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" transition:fade={{ duration: 200 }}>
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-xl bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700" transition:fly={{ y: -20, duration: 200 }}>
      <div class="mt-3 text-center">
        <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full {confirmActionType === 'disable' ? 'bg-red-100 dark:bg-red-900/30' : 'bg-green-100 dark:bg-green-900/30'}">
        ...
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mt-4">
          Confirm {confirmActionType === 'disable' ? 'Disable' : 'Enable'} User
        </h3>
        <div class="mt-2 px-7 py-3">
          <p class="text-sm text-gray-500 dark:text-gray-400">
            Are you sure you want to {confirmActionType} the user account for <strong class="text-gray-900 dark:text-white">{confirmUserEmail}</strong>?
          </p>
          {#if confirmActionType === 'disable'}
        ...
        <div class="items-center px-4 py-3">
          <div class="flex justify-center space-x-3">
            ...
            <form method="POST" action="?/toggleUserStatus" use:enhance={() => {
              ...
              <button
                type="submit"
                class="px-4 py-2 {confirmActionType === 'disable' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'} text-white text-base font-medium rounded-lg shadow-sm focus:outline-none focus:ring-2 {confirmActionType === 'disable' ? 'focus:ring-red-500' : 'focus:ring-green-500'} cursor-pointer transition-colors duration-200"
              >
                {confirmActionType === 'disable' ? 'Disable' : 'Enable'}
              </button>
            </form>
          </div>
        </div>
