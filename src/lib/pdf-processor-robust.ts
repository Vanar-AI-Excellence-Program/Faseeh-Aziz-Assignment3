...
/**
 * Robust PDF processor using pdf-lib and pdf-extraction
...
export class RobustPDFProcessor {
  ...
  /**
   * Main method to extract text from PDF with robust fallback strategies
   */
  async extractTextFromPDF(buffer: Buffer): Promise<PDFExtractionResult> {
    ...
    console.log(`📄 Processing PDF with robust processor: ${buffer.length} bytes`);
    ...
    const methods = [
      ...
      { name: 'pdf-lib-text', fn: () => this.extractWithPDFLibText(buffer) },
      { name: 'pdf-lib-metadata', fn: () => this.extractWithPDFLibMetadata(buffer) },
      { name: 'fallback-analysis', fn: () => this.extractWithFallbackAnalysis(buffer) }
    ];
    ...
    throw new Error(`All robust extraction methods failed. Last error: ${lastError}`);
  }

  /**
   * Method 1: Extract using pdf-extraction (based on pdf.js)
   */
  ...
  /**
   * Method 2: Extract text using pdf-lib with text extraction
   */
  private async extractWithPDFLibText(buffer: Buffer): Promise<PDFExtractionResult> {
    try {
      // Import pdf-lib
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        // Try to get text content from page
        try {
          // Note: pdf-lib doesn't directly support text extraction, but we can get page info
          extractedText += `Page ${i + 1} content available.\n`;
        } catch (textError) {
          extractedText += `Page ${i + 1} text extraction not available.\n`;
        }
      }
      ...
      throw new Error(`pdf-lib text extraction failed: ${error.message}`);
    }
  }

  /**
   * Method 3: Extract metadata and structure using pdf-lib
   */
  private async extractWithPDFLibMetadata(buffer: Buffer): Promise<PDFExtractionResult> {
    try {
      // Import pdf-lib
      const { PDFDocument } = await import('pdf-lib');
      ...
      return {
        text: this.cleanTextEnhanced(extractedText),
        pages: pageCount,
        method: 'pdf-lib-metadata',
        confidence: 0.6
      };
    } catch (error: any) {
      throw new Error(`pdf-lib metadata extraction failed: ${error.message}`);
    }
  }

  /**
   * Method 4: Fallback analysis for complex PDFs
   */
  private async extractWithFallbackAnalysis(buffer: Buffer): Promise<PDFExtractionResult> {
    try {
...
export const robustPDFProcessor = new RobustPDFProcessor();
