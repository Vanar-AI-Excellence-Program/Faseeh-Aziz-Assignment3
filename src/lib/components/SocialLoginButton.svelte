<script lang="ts">
  const { provider, action, callbackUrl } = $props<{ 
    provider: string; 
    action: string; 
    callbackUrl: string; 
  }>();
  ...
  const providerConfig = {
    google: {
      ...
      hoverBgColor: 'hover:bg-slate-50 dark:hover:bg-slate-600'
    },
    github: {
      name: 'GitHub',
      ...
      hoverBgColor: 'hover:bg-slate-800 dark:hover:bg-slate-100'
    }
  };
...
<a 
  href="/auth/{provider}?callbackUrl={encodeURIComponent(callbackUrl)}"
